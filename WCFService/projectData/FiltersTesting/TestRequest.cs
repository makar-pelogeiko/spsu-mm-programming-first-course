...
namespace FiltersTesting
{
    class TestRequest
    {
        ...
        public TestRequest(int totalRequests, byte[] testImage, string resultpath, string ipConnect)
        {
        ...
        public TestRequest(int totalRequests, byte[] testImage, string resultpath, string titleTest, string ipConnect)
        {
            pathResult = resultpath;
            requestAmount = totalRequests > 1 ? totalRequests : 1;
            image = testImage;
        ...
        public int MakeTest()
        {
            ...
            for (int i = 0; i < requestAmount; ++i)
            {
                ...
                var factory = new ChannelFactory<IContract>(binding, new EndpointAddress($"net.tcp://{ip}:12345/srv"));
