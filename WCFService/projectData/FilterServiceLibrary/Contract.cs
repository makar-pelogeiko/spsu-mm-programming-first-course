...
namespace FilterServiceLibrary
{
    ...
    public class Contract : IContract
    {
        ...
        private void RunCmdExtendedFilter(string fileIn, string filter, string fileOut, string parametrs)
        {
            string program = "filtersC.exe";
            string command = $"{program} {fileIn} {filter} {fileOut} {parametrs}";
            Process cmd = new Process();
            cmd.StartInfo.FileName = "cmd.exe";
            cmd.StartInfo.RedirectStandardInput = true;
            cmd.StartInfo.RedirectStandardOutput = true;
        ...
        public byte[] ApplyExtendedFilter(string[] filter, byte[] image)
        {
            progress = 0;
            //Console.WriteLine("ApplyFilter is running");
            str = filter[0];
            ...
            try
            {
                ...
                RunCmdExtendedFilter(fileIn, filter[0], fileOut, filter[1]);
                Bitmap doneImage = new Bitmap(pathFile + fileOut);
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    doneImage.Save(memoryStream, System.Drawing.Imaging.ImageFormat.Bmp);
